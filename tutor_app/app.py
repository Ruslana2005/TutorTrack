...
# ---------- ОЦЕНКА ЗА ДОМАШНЕЕ ЗАДАНИЕ (КУБКИ) ----------
@app.post("/homework/{homework_id}/grade")
async def grade_homework(
    ...
    for student in homework.lesson.students:
        total = db.query(func.sum(Homework.points)).join(Lesson).filter(
            Lesson.students.any(Student.student_id == student.student_id)
        ).scalar() or 0
        ...
        if not achievement:
            achievement = StudentAchievement(student_id=student.student_id)
            db.add(achievement)
        
        achievement.total_points = total
...
async def achievements(request: Request, db: Session = Depends(get_db)):
    ...
    homeworks = db.query(Homework).join(Lesson).filter(Lesson.students.any(Student.student_id == current_user.student_id)).all()
    total_points = sum(h.points or 0 for h in homeworks)
...
async def shop(request: Request, db: Session = Depends(get_db)):
    ...
    total_points = db.query(func.sum(Homework.points)).join(Lesson).filter(
        Lesson.students.any(Student.student_id == current_user.student_id)
    ).scalar() or 0
